...
function AutoUpdate(data)
    if tonumber(data) > tonumber(ver) then
        ...
        DownloadFileAsync("https://raw.githubusercontent.com/LetsRoar/Scripts/master/GOS/RoarRiven.lua", SCRIPT_PATH ..
...
-- W cancel event
  if GetObjectType(unit) == Obj_AI_Hero and GetTeam(unit) ~= GetTeam(myHero) and IsReady(_W) then
    if CHANELLING_SPELLS[spell.name] then
      if ValidTarget(unit, 240) and GetObjectName(unit) == CHANELLING_SPELLS[spell.name].Name and RivenMenu.Interrupt[GetObjectName(unit).."Inter"]:Value() then
      CastSpell(_W)
      end
    end
  end
    if MinionsAround(myHero,260, MINION_JUNGLE) then AttackUnit(mob) end
  if unit == myHero then
      ...
      if  Mode()=="LaneClear" then
                ...
                for _, minion in pairs(minionManager.objects) do
                    if GetTeam(minion) == MINION_ENEMY then
                        if RivenMenu.LaneClear.LCH:Value() and Ready(_Q) and ValidTarget(minion, 260) then
                        CastSpell(_Q)
                        elseif RivenMenu.LaneClear.LCW:Value() and Ready(_W) and ValidTarget(minion, 250) then
                        CastSpell(_W)
                        elseif RivenMenu.LaneClear.LCH:Value() and Tiamat>0 and ValidTarget(minion, 350) then
                        CastSpell(Tiamat)
                        elseif RivenMenu.LaneClear.LCH:Value() and Hydra>0 and ValidTarget(minion, 350) then
                        CastSpell(Hydra)
                        end
                    end
				end
				...
				for _, mob in pairs(minionManager.objects) do
                    if GetTeam(mob) == MINION_JUNGLE then
                        if RivenMenu.JungleClear.JCQ:Value() and Ready(_Q) and  ValidTarget(mob, 260)then
                        CastSpell(_Q)
                        elseif RivenMenu.JungleClear.JCW:Value() and Ready(_W) and ValidTarget(mob, 250) then
                        CastSpell(_W)
                        elseif RivenMenu.JungleClear.JCH:Value() and Tiamat>0 and ValidTarget(mob, 350) then
                        CastSpell(Tiamat)
                        elseif RivenMenu.JungleClear.JCH:Value() and Hydra>0 and ValidTarget(mob, 350) then
                        CastSpell(Hydra)
                        elseif RivenMenu.JungleClear.JCE:Value() and Ready(_E) and ValidTarget(mob, 350) then
                        CastSpell(_E)
                        end
                    end
